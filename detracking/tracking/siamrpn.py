...
class SiamRPN(nn.Module):
    ...
    def learn(self, z):
        # z shape: [1,512, 6, 6] | kernel_reg shape: [1,10240,4,4] | kernel_cls shape :[1,5120,4,4]
        z = self.feature(z)
        kernel_reg = self.conv_reg_z(z)
        kernel_cls = self.conv_cls_z(z)

        k = kernel_reg.size()[-1]
        kernel_reg = kernel_reg.view(4 * self.anchor_num, 512, k, k) # [20, 512, 4, 4]
        kernel_cls = kernel_cls.view(2 * self.anchor_num, 512, k, k) # [10, 512, 4, 4]

        return kernel_reg, kernel_cls

    def inference(self, x, kernel_reg, kernel_cls):
        x = self.feature(x)
        # x :[1, 512, 24, 24] | x_reg and x_cls shape are [1,512, 22, 22]
        x_reg = self.conv_reg_x(x)
        x_cls = self.conv_cls_x(x)

        # out_reg [1,20,19,19] | out_cls [1,10,19,19]
...
class TrackerSiamRPN(Tracker):
    ...
    def init(self, image, box):
        ...
        self.hann_window = np.outer(
            ...
            np.hanning(self.response_sz))
        # np,tile(a,2) --> [0,1,2,0,1,3]
        self.hann_window = np.tile(
            self.hann_window.flatten(),
            len(self.cfg.ratios) * len(self.cfg.scales))

        # exemplar and search sizes
        # context : shang xia fudong
        context = self.cfg.context * np.sum(self.target_sz)
        self.z_sz = np.sqrt(np.prod(self.target_sz + context)) # get z search size
        # get x search size
        self.x_sz = self.z_sz * \
